      ...
      <table className={`w-full text-sm mb-4 whitespace-pre-wrap`}>
        <tbody>
          <Row
            ...
            value={
              <Link
                href={`https://${result.domain || target}`}
                target="_blank"
                rel="noopener noreferrer"
                className="text-primary hover:underline transition-all duration-200 inline-flex items-center"
              >
                {result.domain || target.toUpperCase()}
                <RiExternalLinkLine className="w-3 h-3 ml-1 opacity-60" />
              </Link>
            }
            badge={
